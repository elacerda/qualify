...
a 32 milhões de anos, escolhida pois é a que possui o melhor coeficiente de correlação entre os
...
correlação não altera substancialmente entre $10^7$ e $10^{7.5}$.
...
painéis}: Cada um dos gráficos de comparação entre as SFR utilizando $t_\star = t_{SF} = 32 M anos$.
Em cada painel a linha pontilhada vermelha é o ajuste linear utlizando {\em OLS bisector}, em azul é o
ajuste forçando que a inclinação seja 1 e em preto é a bissetriz ($x = y$). Os números no canto
...
0.44 \tauVN$). \citet{Charlot.Fall.2000a} propõe um modelo onde populações jovens são atenuadas
pela núvem de gás que a gerou ({\em birth cloud} - BC) e encontra que o coeficiente de extinção das
BC é cerca de 3 vezes maior que o do meio interestelar ({\em interstellar medium} - ISM).
\citet{Kreckel.etal.2013a} através de dados de espectrografia de campo integrado (como o CALIFA) do
...
\begin{figure}
	\centering
	%\resizebox{0.99\textwidth}{!}{\includegraphics{Nh_logt_metBase_Padova2000_salp.pdf}}
	\resizebox{0.99\textwidth}{!}{\includegraphics{figuras/CompareTauV_realsample.pdf}}
...
\begin{figure}
	\centering
	%\resizebox{0.99\textwidth}{!}{\includegraphics{Nh_logt_metBase_Padova2000_salp.pdf}}
	\resizebox{0.99\textwidth}{!}{\includegraphics{figuras/stellar_muZR_realsample.pdf}}
...
Em \ref{fig:compareZR} e \ref{fig:compareZint} vemos em detalhe a comparação entre as metalicidades
...
\begin{figure}
	\centering
	\resizebox{0.99\textwidth}{!}{\includegraphics{figuras/CompareZR.pdf}}
...
\begin{figure}
	\centering
	\resizebox{0.99\textwidth}{!}{\includegraphics{figuras/CompareZint.pdf}}
...
gradientes negativos de \meanM{\log Z_\star} menos inclinados que os previstos por modelos de
crescimento isolado. Galáxias do tipo Sb-Sbc possuem gradientes de \meanM{\log Z_\star} compatíveis
