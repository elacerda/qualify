...
def plotBPT(ax, O3Hb, N2Ha, z = None, cmap = 'viridis', mask = None):
    if mask is None: mask = np.zeros_like(O3Hb, dtype = np.bool_)
    from CALIFAUtils.lines import Lines
    plt.axis([-1, 0.5, -1, 1])
    if z is None:
        sc = ax.scatter(N2Ha[mask], O3Hb[mask], marker = 'o', s = 30, edgecolor = 'none')#, alpha = 0.6)
    else:
        sc = ax.scatter(N2Ha[mask], O3Hb[mask], c = z[mask], cmap = cmap, marker = 'o', s = 30, edgecolor = 'none')#, alpha = 0.6)
        cb = plt.colorbar(sc, ticks = [0, .5, 1, 1.5, 2])
        cb.set_label('R [HLR]', fontsize = 15)
    trans_angle = plt.gca().transData.transform_angles(np.array((np.arctan(1.01) * 180 / np.pi,)), np.array((-1, 0.5)).reshape((1, 2)))[0]
...
ax = f.gca()
ax = plotBPT(ax, O3Hb__z, N2Ha__z, K.zoneDistance_HLR, 'viridis_r', mask)
f.subplots_adjust(left = 0.2, right = 0.90, top = 0.95, bottom = 0.15)
