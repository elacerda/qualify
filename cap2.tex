...
Todos os elementos, além do hidrogênio e do hélio, são considerados metais em astrofísica.
...
\citep[{\em strong-line methods; }][]{Pagel.etal.1979a}, principalmente os que utilizam as razões
...
\Hbeta, \OIII e \NII) pode ser visto na Fig. \ref{fig:BPTprog} e sua saída resulta na Fig.
...
gráfico é feito utilizando a biblioteca gráfica M\textsc{atplotlib} \footnote{
...
\begin{figure}
	...
	{Diagrama BPT para as regiões da galáxia NGC29116 (objeto CALIFA 277) produzida pelo exemplo da
Fig. \ref{fig:BPTprog}. As cores marcam a distância ao centro da galáxia em unidades do raio que
contém metade da luz (HLR). Nela utiliza-se os fluxos de \Halpha, \Hbeta, \OIII e \NII.}
	\label{fig:BPTfig}
\end{figure}
...
genérica \emph{por unidade de massa formada} (portanto, dependente da Função de Massa
Inicial ({\em Initial Mass Function} - IMF)) de uma população estelar simples ({\em
...
\begin{eqnarray}
	...
	\label{eq:dM_t}
	\Lambda(T_U) &=& \int_0^{T_U} \psi(t)\ l(t)\ \textrm{d}t
	\label{eq:Lambda}
\end{eqnarray}
...
número de fótons H-ionizantes rapidamente converge ao máximo perto de $t = 10^7$ anos. Utilizando
\eqref{eq:dM_t} em \eqref{eq:QH_t} e com uma simplificação graças a SFR constante dentro desta
escala temporal ($\psi(t)\rightarrow \psi$), podemos reescrever \eqref{eq:QH_t} usando
\eqref{eq:Nh}:
\begin{equation}
